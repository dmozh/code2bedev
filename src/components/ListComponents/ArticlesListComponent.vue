            ...
            <div class="post-content">
              <div class="post-header">
                <div class="title">
                  ...
                  <div id="title">{{elem.article_name}}</div>

                </div>
                <div class="tags">
                  <div>Тэги:&nbsp;</div>
                  <span v-for="(tag, index) in elem.article_tags" :key="index" class="tag" @click="setSortKey(tag)">#{{tag}}</span>
                </div>
              </div>
              <div class="post-body">
                <p class="txt">{{elem.article_description}}</p>
              </div>
              <div class="post-footer">
                ...
                <div class="cnt ft">
                  <p class="info" v-if="elem.article_rate < 0">Рейтинг <span class="s minus-rate">{{elem.article_rate}}</span></p>
                  <p class="info p" v-else-if="elem.article_rate > 0">Рейтинг <span class="s plus-rate">{{elem.article_rate}}</span></p>
                  <p class="info"   v-else>Рейтинг <span class="s">{{elem.article_rate}}</span></p>
    ...
    export default {
            ...
            return this.response.filter(
              function (article) {
                if(this.sortKey==='Название'){
                  return article.article_name.indexOf(this.search) !== -1
                }else if(this.sortKey==='Тэги'){
                  return article.article_tags.indexOf(this.search) !== -1
                }
      ...
      methods: {
        ...
        async getPosts(){
          ...
          const jBody = JSON.stringify(body);
          await axios.post(this.$root.URL+'getArticles', jBody).then((response) => {
            this.response = response.data.articles;
            // console.log(response);
          }).catch((error) => {
            console.log(error);});
          // this.added_time_date = this.response.added_time;
